      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          {projects.map((project, i) => (
            ...
            >
              <div className={`glass-card rounded-2xl p-7 md:p-10 transition-all duration-500 bg-gradient-to-br ${project.gradient} h-full`}>
                {/* Top row */}
                <div className="flex items-start justify-between mb-8">
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] uppercase tracking-[0.25em] text-white/25">{project.category}</span>
                    {project.live && (
                      <span className="flex items-center gap-1 text-[9px] uppercase tracking-widest text-emerald-400/60">
                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-400/60 inline-block animate-pulse" />
                        Live
                      </span>
                    )}
                  </div>
